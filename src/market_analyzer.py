...
(Provide an offensive/balanced/defensive stance, a position-sizing guideline, one invalidation trigger, and end with "For reference only, not investment advice.")""",
        ...
        if self.region == "us":
            return """### 三、核心趋势与热门权重
（用“上一交易日/昨夜美股”的口径说明核心趋势：大型科技、AI算力、半导体、苹果链、英伟达链、云计算、利率/VIX/美元等；必须点名 Apple、Nvidia、Microsoft、AMD、Broadcom、Meta、Amazon、Tesla、Palantir、SMCI 中与当日主线相关的代表；明确哪些只是指数层面的推断，哪些有新闻或数据支持）

### 四、AI选股雷达
（固定分成三栏：每日热门股、有潜力的股票、可抄底观察信号；每栏最多3个；每个候选必须包含“所属主线、为什么现在关注、次日验证条件、失效条件、风险点”；候选优先从美股核心观察池选择；没有个股行情数据时不得写具体价格、跌幅百分比、均线数值，只能写相对条件）

### 五、明日观察计划
（给出进攻/均衡/防守结论；列出优先观察方向、暂时回避方向、开盘后确认条件和失效条件；目标是帮助快速观察市场，不直接给买入指令）

### 六、风险提示
（列出科技权重、AI高位拥挤、宏观利率、财报/监管等风险；最后补充“建议仅供参考，不构成投资建议”。）"""
...
### 四、涨停板漏斗筛选
（仅 A 股使用：先列出今日涨停板全量池的行业分布和核心题材，再按“涨停逻辑 -> 题材热点 -> 多头趋势/突破平台/资金封板 -> 利空与分时换手风险”逐层剔除；必须先给“筛选结论表”，列出“留下/剔除/待复核”及原因；最后只给 1-2 只“次日重点候选”，必须写触发条件、失效条件、仓位纪律和风险提示，不得写成无条件买入）

### 五、AI选股雷达
（当前市场单独输出三类：每日热门股、有潜力的股票、可抄底观察信号；每类最多3个；每个候选必须按“证据 -> 判断 -> 动作”写清所属主线、观察理由、触发条件、失效条件和风险提示；优先覆盖能反映国内整体局势的热门方向，例如半导体、AI算力、机器人、新能源、消费电子、高端制造、金融/消费、资源；没有个股行情数据时必须标注为“观察池，需量价确认”，不得输出无条件买入，不得编造具体股价/均线数值）
